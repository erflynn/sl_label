...
prefix <- args[1]
setwd(sprintf("data/03_expression/rnaseq/%s/00_infiles/", prefix))
...
data.frame(rp[which(lapply(res, length)==0)]) %>% 
  write_csv(sprintf("../../../../02_metadata/qc/%s_studies_missing_samples.csv",prefix))
data.frame(list_present) %>% 
  write_csv(sprintf("../../../../02_metadata/qc/%s_samples_present.csv", prefix))