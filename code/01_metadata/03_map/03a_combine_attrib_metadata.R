...
#
# TODO:
# - what are the counts without ENA
# - 
...
ae_files <- comb_metadata %>% filter(!str_detect(sample_acc, "GSM|SRR|ERR|DRR") ) 
...
microarray_s <- comb_metadata %>% filter(str_detect(sample_acc, "GSM"))
...
rnaseq_s <- comb_metadata %>% filter(str_detect(sample_acc, "SRR|ERR|DRR"))
...
kept_s <- rnaseq_s %>% filter(present & num_reads > 100000) # 241691
